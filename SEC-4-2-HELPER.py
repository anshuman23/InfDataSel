...
if __name__ == "__main__":
    ...
    influence_test_results = {'util': [pre_test_res['overall_acc']], 'fair': [pre_test_res[args.metric]], 'robust': [pre_test_res['robust_acc']]} #Initialize results dict (infl)
    ...
    args.strategy, args.type = 'inc', 'util' #First experiment, increase utility
    for i,num_i in enumerate(x_ax[1:]):
        args.points_to_delete = int(num_i)

        util_del_idx = deletion_process(args)
        post_val_res, post_test_res = post_main(args)

        influence_test_results['util'].append(post_test_res['overall_acc'])

    args.strategy, args.type = 'inc', 'fair' #Second experiment, increase fairness
    for i,num_i in enumerate(x_ax[1:]):
        args.points_to_delete = int(num_i)
    ...
    args.strategy, args.type = 'inc', 'robust' #Third experiment, increase robustness
    for i,num_i in enumerate(x_ax[1:]):
        args.points_to_delete = int(num_i)

        robust_del_idx = deletion_process(args)
        post_val_res, post_test_res = post_main(args)

        influence_test_results['robust'].append(post_test_res['robust_acc'])
    ...
    for seedval in [42,1,35,999,5454]:
        ...
        for i,num_i in enumerate(x_ax[1:]):
            ...
            random_test_results[args.random_seed]['util'].append(post_test_res['overall_acc'])
            random_test_results[args.random_seed]['fair'].append(post_test_res[args.metric])
            random_test_results[args.random_seed]['robust'].append(post_test_res['robust_acc'])


    # Save experiment results
    with open('sec-4-2-outputs/new_results/influence-'+args.dataset+'-'+args.model_type+'.pkl', "wb") as output_file:
        pickle.dump(influence_test_results, output_file)
    with open('sec-4-2-outputs/new_results/random-'+args.dataset+'-'+args.model_type+'.pkl', "wb") as output_file:
        pickle.dump(random_test_results, output_file)


    # Save influence values
    util_infl_vals = np.load('explainer/data/binaries/util_infl.npy')
    fair_infl_vals = np.load('explainer/data/binaries/fair_infl.npy')
    robust_infl_vals = np.load('explainer/data/binaries/robust_infl.npy')

    deleted_util_influences = {'util':util_infl_vals[util_del_idx], 'fair':fair_infl_vals[util_del_idx], 'robust':robust_infl_vals[util_del_idx]}
    deleted_fair_influences = {'util':util_infl_vals[fair_del_idx], 'fair':fair_infl_vals[fair_del_idx], 'robust':robust_infl_vals[fair_del_idx]}
    deleted_robust_influences = {'util':util_infl_vals[robust_del_idx], 'fair':fair_infl_vals[robust_del_idx], 'robust':robust_infl_vals[robust_del_idx]}

    with open('sec-4-2-outputs/actual_infl/util-'+args.dataset+'-'+args.model_type+'.pkl', "wb") as output_file:
        pickle.dump(deleted_util_influences, output_file)
    with open('sec-4-2-outputs/actual_infl/fair-'+args.dataset+'-'+args.model_type+'.pkl', "wb") as output_file:
        pickle.dump(deleted_fair_influences, output_file)
    with open('sec-4-2-outputs/actual_infl/robust-'+args.dataset+'-'+args.model_type+'.pkl', "wb") as output_file:
        pickle.dump(deleted_robust_influences, output_file)
